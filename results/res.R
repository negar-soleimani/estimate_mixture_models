...
# ---------------------------------------------------------------------
out_dir <- "results/real_data_seuil_gfix"
if (!dir.exists(out_dir)) dir.create(out_dir, recursive = TRUE)

# ---------------------------------------------------------------------
# List of sheets in the Excel file (sheet index -> ball name)
# Index = position of the sheet inside the workbook, used by `read_xlsx`
# ---------------------------------------------------------------------
ball_sheets <- list(
...
# Only init[2] (h0) will be overwritten per drop.
init_template <- c(9.8, NA, 0.01, 0.5, 0.5, 0.1)

# ---------------------------------------------------------------------
# Loop over (sheet, drop)
# ---------------------------------------------------------------------
for (sh in ball_sheets) {
  
  cat("\n============================================================\n")
  cat("Sheet:", sh$idx, "  Ball:", sh$name, "\n")
  cat("============================================================\n")
  ...
  for (dnum in drops_in_sheet) {
    ...
    if (nrow(sub) < 3) {
      cat("  (skipped: not enough observations)\n")
      next
    }
    ...
    # In the original analysis, init[2] is the nominal initial height of
    # the drop (h0 = 46.45 for Blue Basketball, drop 1).  For every
    # other drop, the corresponding value is the maximum recorded height
    # in that drop, taken directly from the data.
    init       <- init_template
    init[2]    <- max(y)
    cat(sprintf("  n = %d  |  t_range = %.4f  |  init[2] = h0 = %.4f\n",
                n, t_range, init[2]))
    
    # ----- Reset RNG so each (sheet, drop) is independent and ------
    #       reproducible
    set.seed(12345)
    
    # ----- MCMC: Scenario IV (g_init = TRUE, seuil = TRUE) ---------
    res <- mcmc_step6(
      y               = y,
      t               = t,
      n_iter          = n_iter,
...
cat("\nAll (sheet, drop) cases finished. Results stored in:\n  ",
    out_dir, "\n")